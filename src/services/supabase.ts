...
export async function logoutUser() {
  if (USE_REAL_BACKEND) await getClient().auth.signOut();
...
export async function updateProfile(userId: string, name: string, avatarUrl = "", vocation = "") {
  if (USE_REAL_BACKEND) {
    await getClient()
      .from("users_profiles")
...
export async function getCandidateDashboardStats(userId: string): Promise<CandidateDashboardStats> {
  if (USE_REAL_BACKEND) {
    ...
    return { vacancies: count, matches: count, accompanimentActive };
  }
  if (userId === "demo-cand") {
...
export async function getAdminCandidates() {
  if (USE_REAL_BACKEND) {
    const { data } = await getClient()
      .from("candidates").select("*, users_profiles!inner(full_name, email, deleted_at)");
    return (data || []).map((c: any) => ({
      user_id: c.user_id, neurotype: c.neurotype,
      work_preference: c.work_preference, interests: c.interests || [],
      users_profiles: c.users_profiles ? { full_name: c.users_profiles.full_name, email: c.users_profiles.email, deleted_at: c.users_profiles.deleted_at } : undefined,
    }));
  }
...
export async function logAdminAction(adminId: string, action: string, targetTable: string, targetId?: string, details?: any) {
  if (USE_REAL_BACKEND) {
    await getClient().from("admin_logs").insert({
      admin_id: adminId, action, target_table: targetTable, target_id: targetId, details: details || {},
    });
  }
}
