...
import { Search, User, Edit, Brain, Filter, Download } from "lucide-react";
import { getAdminCandidates } from "@/services/supabase-admin";
              ...
              {filtered.map(c => (
                <tr key={c.user_id} className={"hover:bg-muted/30 transition-colors " + (c.users_profiles?.deleted_at ? "opacity-50" : "")}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      onClick={() => alert(`Detalles de ${c.users_profiles?.full_name || 'candidato'}`)}
